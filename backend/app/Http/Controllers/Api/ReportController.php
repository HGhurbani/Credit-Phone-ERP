...
class ReportController extends Controller
{
    public function sales(Request $request): JsonResponse
    {
        ...
        $daily = $query->selectRaw('DATE(created_at) as date, COUNT(*) as orders, SUM(total) as revenue')
    ...
    public function collections(Request $request): JsonResponse
    {
        ...
        $daily = $query->selectRaw('payment_date as date, COUNT(*) as payments, SUM(amount) as amount')
    ...
    public function activeContracts(Request $request): JsonResponse
    {
        ...
        $contracts = InstallmentContract::where('tenant_id', $tenantId)
            ->where('status', 'active')
            ->with(['customer', 'branch'])
            ->when($request->branch_id, fn($q) => $q->where('branch_id', $request->branch_id))
            ->paginate($request->per_page ?? 20);

        $summary = InstallmentContract::where('tenant_id', $tenantId)
            ->where('status', 'active')
            ->selectRaw('COUNT(*) as total, SUM(remaining_amount) as total_remaining, SUM(paid_amount) as total_paid')
            ->first();

        return response()->json([
            'summary' => $summary,
            'data' => $contracts->items(),
            'meta' => ['total' => $contracts->total(), 'current_page' => $contracts->currentPage(), 'last_page' => $contracts->lastPage()],
        ]);
    }
    ...
    public function overdueInstallments(Request $request): JsonResponse
    {
        ...
        $schedules = InstallmentSchedule::where('tenant_id', $user->tenant_id)
            ...
            ->when($request->branch_id, fn($q) => $q->whereHas('contract', fn($cq) => $cq->where('branch_id', $request->branch_id)))
            ->orderBy('due_date')
            ->paginate($request->per_page ?? 20);

        $summary = InstallmentSchedule::where('tenant_id', $user->tenant_id)
            ->where('status', 'overdue')
            ->selectRaw('COUNT(*) as total, SUM(remaining_amount) as total_overdue')
            ->first();

        return response()->json([
            'summary' => $summary,
            'data' => $schedules->items(),
            'meta' => ['total' => $schedules->total(), 'current_page' => $schedules->currentPage(), 'last_page' => $schedules->lastPage()],
        ]);
    }
